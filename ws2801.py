...
class ws2801:
 ...
 def colorwipe(self, pixels, c, delay):
	for i in range(len(pixels)):
		self.setpixelcolor_int(pixels, i, c)
 ...
 # the % 96 is to make the wheel cycle around
      		self.setpixelcolor_int(pixels, i, self.Wheel( int((i * 256 / len(pixels)) + j) % 256) )
	   self.writestrip(pixels)
	   time.sleep(wait)
 
 def cls(self, pixels):
          for i in range(len(pixels)):
                self.setpixelcolor_int(pixels, i, self.Color(0,0,0))
                self.writestrip(pixels)
