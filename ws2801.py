...
class ws2801:
        ...
        try:
            with open("/dev/spidev0.0", "wb") as spidev:
                ...
                for i in range(len(pixels)):
                    data.append((pixels[i]>>16) & 0xFF)
                    data.append((pixels[i]>>8) & 0xFF)
                    data.append(pixels[i] & 0xFF)
                spidev.write(data)
            time.sleep(0.002)
        except Exception as e:
            # Suppress error for simulation mode if device not found
            if self.DEBUG:
                # print(f"SPI Error (Simulation?): {e}") 
                pass

    def Color(self, r, g, b):
        return ((int(r) & 0xFF) << 16) | ((int(g) & 0xFF) << 8) | (int(b) & 0xFF)

    def setpixelcolor_rgb(self, pixels, n, r, g, b):
        if (n >= len(pixels)):
            return
    ...
    def setpixelcolor_int(self, pixels, n, c):
         self.setpixelcolor(pixels, n, c)
    ...
    def rainbowCycle(self, pixels, wait):
        for j in range(256): # one cycle of all 256 colors in the wheel
            for i in range(len(pixels)):
                ...
                self.setpixelcolor(pixels, i, self.Wheel( int((i * 256 / len(pixels)) + j) % 256) )
            self.writestrip(pixels)
            time.sleep(wait)
